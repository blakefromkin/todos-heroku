...
app.post("/users/signin", 
  ...
  if (await res.locals.store.validateLogIn(username, password)) {
...
app.post("/lists", requiresAuthentication,
  ...
  catchError( async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        rerenderNewList();
    } else if (await !res.locals.store.isUniqueListTitle(todoListTitle)) {
...
app.post("/lists/:todoListId/todos", requiresAuthentication,
  ...
  catchError(async (req, res) => {
    ...
    let errors = validationResult(req);
    if (!errors.isEmpty()) {
      for (let message of errors.array()) {
      ...
      let addedTodo = await res.locals.store.addNewTodo(+todoListId, todoTitle);
      if (!addedTodo) throw new Error("Not found.");
...
app.post("/lists/:todoListId/edit", requiresAuthentication,
  ...
  catchError(async (req, res) => {
    ...
    try {
      let titleIsUnique = await store.isUniqueListTitle(newTitle);
      if (!titleIsUnique) {
        ...
        reRenderEditList();
          ...
          let renamed = await store.renameTodoList(+todoListId, newTitle);
