...
app.post("/lists",
  ...
  (req, res) => {
    if (!res.locals.store.isUniqueList(req.body.todoListTitle)) {
      ...
      if (!errors.isEmpty()) {
        ...
        res.locals.store.createTodoList(req.body.todoListTitle);
...
app.post("/lists/:todoListId/todos",
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if (!res.locals.store.addNewTodo(+todoListId, todoTitle)) {
        next(new Error("Not found."));
      } else {
...
app.post("/lists/:todoListId/destroy", (req, res, next) => {
  let todoListId = req.params.todoListId;
  if (!res.locals.store.deleteTodoList(+todoListId)) {
    next(new Error("Not found."));
  } else {
    ...
    if (!res.locals.store.isUniqueList(req.body.todoListTitle)) {
      ...
      if (!todoList) {
        ...
        if (!errors.isEmpty()) {
          ...
          if (!res.locals.store.renameTodoList(+todoListId, newTitle)) {
            next(new Error('Not found.'));
          } else {
